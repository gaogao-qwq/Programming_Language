...
from Forms.WithdrawWindow import Ui_withdraw_window
from user import *
import main_menu
import verification_window


class WithdrawWindow(QWidget, Ui_withdraw_window):
    ...
    def on_withdrawButton_clicked(self):
        if float(self.depositTextEdit.toPlainText()) <= 0:
            QMessageBox.warning(self, "喜报", "提现金额不得为 0 或负数", QMessageBox.StandardButton.Ok, QMessageBox.StandardButton.Ok)
            return

        if self.user_list.user_list[self.card_num].balance - float(self.depositTextEdit.toPlainText()) < 0:
        ...
        self.balanceLabel.setText("余额：" + str(self.user_list.user_list[self.card_num].balance))
        QMessageBox.warning(self, "喜报", "提现成功", QMessageBox.StandardButton.Ok, QMessageBox.StandardButton.Ok)
